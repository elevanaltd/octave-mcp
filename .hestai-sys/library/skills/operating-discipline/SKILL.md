...
QUALITY_GATES::"TDD(failing_test_first)→TMG_CHECKPOINT(T2+:TMG_must_APPROVE)→CodeReview(every_change)→Tests(must_pass)→Security(scan_required)"
...
QUALITY_GATES::TDD[failing_test_first]→TMG[T2+_TMG_must_APPROVE]→CodeReview[every_change]→Tests[must_pass]→Security[scan_required]
