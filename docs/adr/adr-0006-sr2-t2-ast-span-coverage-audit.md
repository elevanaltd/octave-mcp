...
## Post-CDV revisions log (2026-05-12)

Following CDV B0 gate review on PR #409, the following revisions were applied before PR-2 (T4–T7) opens:

- **P0-1** (§3 + §6 + §7): schema repair sites in `core/repair.py` and `mcp/write.py:3618-3633` added to lenient-repair list; T4 linter gate extended; T4 LOC band 200–250 → 250–320.
- **P0-2** (§5 + §7): "fixed point" framing in §5 ¶3 rewritten; R4 debug-assertion replaced with a structural integrity check (no parse∘emit identity dependency).
- **P1-2** (§3): `Document.trailing_comments` span policy specified (band between last section end and `===END===`).
- **P2-3** (§4 + §6 + §7): `meta_dirty` moved from document-level bool to per-key dict; T5 LOC band 150–250 → 200–300.
- **LOC envelope** revised from audit's 1,580–2,500 to **1,820–2,800** per CDV's task-by-task reassessment. Stabilising trend (audit was +2× over original brief; CDV is +13% over audit).
- **PR-1 (T1+T2+T3) merged as #412**; ADR §7 R3 INDENT convention reconciled (implementation chose "covers leading spaces" over brief's "zero-width"; test renamed accordingly).
...
`Document.trailing_comments` (cst.py:240) are owned by the Document; their byte band lies between `doc.sections[-1].end_byte` and the `===END===` token. When `doc.dirty=False` AND `Document.trailing_comments` is structurally unchanged, the entire trailing region slices unchanged from baseline. When either is dirty, the band re-emits through emitter.py:850.
...
### Schema repairs (post-parse mutation sites outside parser.py)

**Schema repairs (core/repair.py `_apply_schema_repairs` plus mcp/write.py enum-casefold at 3618-3633)**: every `node.value = ...` mutation and every `doc.meta[k] = ...` outside the parser proper must paired-write to `dirty`/`repaired`. The T4 linter gate must cover both `mcp/write.py` AND `core/repair.py`. Reasoning identical to parser.py lenient repairs: splicing a repaired node would re-introduce the source bytes that the repair just normalised away — an I1 violation.
...
5. Add to the list of node-attribute fields: `Document.meta_dirty: dict[str, bool] = {}` — a per-key dirty map. Setting `doc.meta_dirty["STATUS"] = True` marks only that META key for re-emission; all other META keys slice unchanged from baseline. This keeps META.STATUS edits at ~30 bytes of diff footprint regardless of META block size. (Rejected alternative: document-level `meta_dirty: bool` would force whole-META re-emission and blow the 0.5% target on META-heavy files.)
...
| META.FIELD update (write.py:2741) | Document | `doc.meta_dirty[FIELD] = True` (per-key dirty map; other META keys slice unchanged). |
...
### I1 implications
Option A keeps the canonical wire form as a function of (AST, FormatOptions). Strategy A's correctness rests on `baseline_bytes[start:end]` being **the source bytes themselves** — NOT on parse∘emit being an identity function. The emitter today is intentionally not a fixed point: canonical dequoting at emitter.py:528, comment-prefix canonicalisation at emitter.py:438 (`//foo` → `// foo`), trailing-comment spacing normalisation. Strategy A's slice path is correct *because it returns source bytes verbatim*; the re-emit path is correct because it produces canonical bytes that the user has explicitly opted into via `dirty=True`. Two truths, never compared against each other.
...
**Revised LOC envelope: 1,580–2,500 across T1–T11**, of which test code is ~250–450. The brief's "800–1200" estimate was optimistic by roughly 2×; the under-estimate concentrates in T3 (parser propagation) and T8 (emitter integration), both of which have larger surface than the headline node-count suggests. Revised envelope: **1,820–2,800 LOC** post-CDV.
...
| **R4 — Emit-time span invalidation** | New risk introduced by Strategy A. | Medium × Medium | If upstream code mutates AST after spans are computed but before emit (e.g. a future normalize-pass), the span is stale. | Span points into baseline bytes that no longer represent the node. | Mark every post-parse pass that mutates AST as a `dirty=True` site by policy. Pre-emit assertion: `node.dirty or (node.start_byte is None) or (node.end_byte > node.start_byte and 0 <= node.start_byte < len(baseline_bytes))` — a structural integrity check, not a parse∘emit identity check. Detects span-out-of-range or inverted-range bugs; does NOT mistakenly fire on legitimate canonical drift. Gated behind a debug flag, not prod. (T8) |
| **R5 — Diff footprint regression under blank-line edits** | New risk. | Low × Medium | User edits a value that is followed by trailing blank lines; trail-anchored policy means the next node is unaffected. Also: per-key `Document.meta_dirty: dict[str, bool]` ensures single-META-key edits only re-emit that key — not the whole META band. | None — this is the policy working as designed. Flag here to document the deliberate non-regression. | Section 3 trail-anchoring policy + Section 4 per-key meta_dirty design. Test in T9 includes blank-line cases and a META.STATUS single-key edit on a META-heavy fixture. |
...
| **R7 — Lenient-mode trip-wires** | Re-confirmed from PR-A R6. | Medium × High | W002 / multi_word_coalesce / curly-brace repair (parser.py:1283,1368,1427 plus tokenize at lexer.py:805) all mutate at parse. Additionally: schema repair sites in `repair.py` (`_apply_schema_repairs`, `_repair_ast_node`) and the enum-casefold branch at `mcp/write.py:3618-3633` mutate post-parse outside the parser proper. | If `repaired:bool` is forgotten on a repair site, that site silently violates I1 under Strategy A. | T4 explicit checklist over every entry in `parser.py:warnings.append({"type": "lenient_parse", …})` plus every value mutation in `core/repair.py` and `mcp/write.py:3618-3633` — every such site must set `repaired=True` on the affected node. CI grep gate covers parser.py, repair.py, AND write.py. |
| **R3a — INDENT convention** | Reconciled in PR-1 (#412) | n/a (resolved) | INDENT tokens cover leading whitespace bytes (lexer.py:25-26 implementation chose this over the brief's "zero-width INDENT"). | Internally consistent; more useful for splice operations. | Documentation reconciled; test renamed at #412 `65579c2`. |

**New risks specific to Strategy A: R2, R3, R4, R5.** R1, R6, R7 are PR-A diagnostic risks confirmed/sharpened. R3a is a documentation reconciliation against the implementation choice in PR-1.
